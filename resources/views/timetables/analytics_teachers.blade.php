...
<div style="max-width:1200px;margin:0 auto;padding:20px;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
        <div>
            ...
            <h1 style="margin:10px 0 0;font-size:38px;font-weight:900;">Teacher Analytics — Run #{{ $runId }}</h1>
        </div>
    ...
    <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:16px;">
        <div style="border:1px solid #e5e7eb;border-radius:12px;padding:12px 14px;background:#fff;">
            Teachers: <b>{{ $summary['total_teachers'] ?? $loads->count() }}</b>
        </div>
        <div style="border:1px solid #e5e7eb;border-radius:12px;padding:12px 14px;background:#fff;">
            Total lessons: <b>{{ $summary['total_lessons'] ?? 0 }}</b>
        </div>
        <div style="border:1px solid #e5e7eb;border-radius:12px;padding:12px 14px;background:#fff;">
            Avg load: <b>{{ $summary['avg_load'] ?? 0 }}</b>
        </div>
        <div style="border:1px solid #e5e7eb;border-radius:12px;padding:12px 14px;background:#fff;">
            Balanced: <b>{{ $summary['balanced'] ?? 0 }}</b>
        </div>
        <div style="border:1px solid #e5e7eb;border-radius:12px;padding:12px 14px;background:#fff;">
            Overloaded: <b>{{ $summary['overloaded'] ?? 0 }}</b>
        </div>
        <div style="border:1px solid #e5e7eb;border-radius:12px;padding:12px 14px;background:#fff;">
            Underloaded: <b>{{ $summary['underloaded'] ?? 0 }}</b>
        </div>
    </div>
        ...
        <table style="width:100%;border-collapse:collapse;min-width:900px;">
            <thead>
                <tr style="background:#f9fafb;">
                    <th style="text-align:left;padding:10px;border-bottom:1px solid #e5e7eb;">Teacher ID</th>
            ...
            @forelse($loads as $r)
                <tr>
                    <td style="padding:10px;border-bottom:1px solid #f1f5f9;">{{ $r->teacher_id }}</td>
                    ...
                    <td style="padding:10px;border-bottom:1px solid #f1f5f9;">
                        ...
                        <span style="padding:4px 10px;border-radius:999px;border:1px solid #e5e7eb;background:#f9fafb;">
                            {{ $st }}
                        </span>
                    </td>
                </tr>
            @empty
                <tr><td colspan="5" style="padding:14px;color:#6b7280;">No data.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
